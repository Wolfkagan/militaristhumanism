...
export function privacyPage(session: AppSession | null): string {
  return layout({
    ...
    session,
    body: `<article class="prose page-article"><p class="eyebrow">Privacy by restraint</p><h1>Privacy and retention</h1><h2>Site analytics</h2><p>Cloudflare Web Analytics measures aggregate visits and performance. No advertising tracker or browser fingerprinting is used.</p><h2>Community account data</h2><p>OAuth provides identity. Secure server-managed sessions expire. Provider access and refresh tokens are encrypted at rest; identity tokens are not retained after sign-in.</p><h2>Public writing</h2><p>Discussions and replies remain public until their author deletes them or a moderator hides them. Deleted text is removed from public view.</p><h2>Moderation and security</h2><p>Moderation reasons and restricted evidence are retained for accountability. Rate-limit and application telemetry is aggregated. Better Auth IP tracking is disabled, legacy session IP fields are cleared, and the application stores only a short-lived HMAC-derived rate-limit key rather than a raw visitor address.</p><h2>Retention</h2><p>Notifications are retained for up to 180 days, operational aggregates for 90 days, sessions until expiry or revocation, and cryptographically sealed moderation audit records for accountability.</p></article>`,
  });
}
